...
pthread_mutex_t guess_queue_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t guess_queue_notfull = PTHREAD_COND_INITIALIZER; // wait until queue isn't full (worker)
pthread_cond_t guess_queue_notempty = PTHREAD_COND_INITIALIZER; // wait until queue isn't empty (prefetcher)
request_queue_t requests[MAX_QLEN];
request_queue_t guesses[MAX_QLEN];
int req_remove_index = 0;
int req_insert_index = 0;
int guess_remove_index = 0;
int guess_insert_index = 0;
int req_num = 0;
int guess_num = 0;
...
void * worker(void *arg) {
   ...
   while (1) {
	 ...
	 close(filefd);
      ...
      // Put the request back to prefetch queue
      //printf("Putting %s in guess queue\n", mybuf);
      pthread_mutex_lock(&guess_queue_mutex);
      if (guess_num != qlen){
          guesses[guess_insert_index].fd = fd;
          guesses[guess_insert_index].request = mybuf;
          guess_num++;
          guess_insert_index = (guess_insert_index + 1) % qlen; // ring buffer
          pthread_cond_signal(&guess_queue_notempty);
          pthread_mutex_unlock(&guess_queue_mutex);
      }
   }
   /*******************************************************/
      ...
      for (i = req_remove_index; i < qlen; i++){
	      ...
	      if (requests[index].request != NULL){
		   ...
		   //printf("-%d- Request %s\n", id,mybuf);
		       ...
		       found = -1;
                       if (cache[cache_index].valid == 1 && strcmp(mybuf, cache[cache_index].request) == 0){
			       found = cache_index;
...
void* prefetcher(void* arg){
     int id = (int)arg;
     int fd,i, filesize, myreqnum = 0;
     char guessed[1024];
     void * memory;
     char mybuf[1024];
     char *hit_or_miss = "MISS";
    ...
    // get request from queue
    pthread_mutex_lock(&guess_queue_mutex);
    while (guess_num == 0) // the queue is empty!
        pthread_cond_wait(&guess_queue_notempty,&guess_queue_mutex);

    fd = guesses[guess_remove_index].fd;
    strcpy(mybuf,guesses[guess_remove_index].request);
    if (strcmp(mybuf,"/")==0){
         strcpy(mybuf,"/index.html");
    }
    //free(guesses[guess_remove_index].request);
    guess_num--;
    guess_remove_index = (guess_remove_index + 1) % qlen; // ring buffer
    pthread_mutex_unlock(&guess_queue_mutex);
    if (nextguess(mybuf, guessed)){
        pthread_mutex_lock(&log_mutex);
        fprintf(logfile,"[%d][%d][%d][%s][%s][%s]\n",id,myreqnum,fd,hit_or_miss,mybuf,"Invalid prefetch request");
        fflush(logfile);
        pthread_mutex_unlock(&log_mutex);
        continue;
    }
    pthread_mutex_lock(&cache_mutex);
    for (i = 0; i < cache_entries; i++){
	if (cache[i].valid == 1 && strcmp(guessed,cache[i].request) == 0) {
            found_in_cache = 1;
            break;
        }
     }
...
//	 printf("File %s NOT in cache\n", mybuf);
	 hit_or_miss = "MISS";
         ...
         if ((filefd = open(mybuf+1,O_RDONLY)) == -1) {
          ...
          read(filefd,memory,filesize);
          close(filefd);
          if (caching != 0) { // insert only if caching is enabled (non-zero)
	  ...
	  pthread_mutex_unlock(&cache_mutex);
          pthread_mutex_lock(&log_mutex);
         fprintf(logfile,"[%d][%d][%d][%s][%s][%s]\n",id,myreqnum,fd,hit_or_miss,mybuf,guessed);
         fflush(logfile);
         pthread_mutex_unlock(&log_mutex);
     }
...
int main(int argc, char **argv) {
   ...
   pthread_t prefetch_threads[MAX_THREADS];

   logfile = fopen("webserver_log","w");

   if (argc != 8) {
      printf("usage: %s port path num_dispatch num_workers qlen mode cache-entries\n",argv[0]);
      return -1;
   }
   ...
   for (i=0; i < 10; i++) {
      pthread_create(prefetch_threads+i,NULL,prefetcher,(void *)i);
   }
   ...
   // the prefetchers should be able to exit now.
   for (i=0; i < 10; i++) {
