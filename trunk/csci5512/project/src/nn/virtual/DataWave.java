...
public class DataWave extends VirtualWave {
   ...
   public DataWave(double startX, double startY, double heading, double firePower, long creationTime, Vector data,
         RobotData view, RobotData reference) {
      super(startX, startY, heading, firePower, creationTime);
      init(data, view, reference);
   }

   public DataWave(Bullet bullet, RobotData firer, RobotData target, Vector data, RobotData view, RobotData reference) {
      super(firer.getX(), firer.getY(), Utils.angle(firer.getX(), firer.getY(), target.getX(), target.getY()), bullet
            .getPower(), firer.getTime());
      init(data, view, reference);
   }

   public DataWave(RobotData firer, RobotData target, double firePower, Vector data, RobotData view, RobotData reference) {
      super(firer.getX(), firer.getY(), Utils.angle(firer.getX(), firer.getY(), target.getX(), target.getY()),
            firePower, firer.getTime());
      init(data, view, reference);
   }

   public DataWave(Robot firer, RobotData target, double firePower, Vector data, RobotData view, RobotData reference) {
      super(firer.getX(), firer.getY(), Utils.angle(firer.getX(), firer.getY(), target.getX(), target.getY()),
            firePower, firer.getTime());
      init(data, view, reference);
   }

   private void init(Vector data, RobotData view, RobotData reference) {
      this.data = data;
      this.view = view;
