...
public class Shooter extends AdvancedRobot {
   ...
   private AdvancingVelocity   advancingVelocity = new AdvancingVelocity(9);
   private Acceleration        acceleration      = new Acceleration();
   private LateralVelocity     lateralVelocity   = new LateralVelocity(11);
   ...
   public void run() {
      ...
      while (true) {
         ...
         if (profile_ != null) {
            Vector data = profile_.getOutput();

            double[] sum = new double[data.getLength()];
            for (int i = 0; i < sum.length; i++) {
               int b = Utils.limit(0, i - 1, data.getLength() - 1);
               int a = Utils.limit(0, i + 1, data.getLength() - 1);
               sum[i] = 0.2 * data.get(b) + 0.6 * data.get(i) + 0.2 * data.get(a);
            }
            ...
            angle = Utils.angle(getX(), getY(), enemy_.getX(), enemy_.getY())
                  + Utils.getAngleOffset(this, enemy_, Utils.getGuessFactor(index, data.getLength()), 3.0);
         }
   ...
   public void onPaint(Graphics2D g) {
      ...
      distance.draw(grid, enemy_, new RobotData(this), 20, 20, 150, 20);

      grid.setColor(Color.GREEN);
      advancingVelocity.draw(grid, enemy_, new RobotData(this), 200, 20, 100, 20);

      grid.setColor(Color.YELLOW);
      acceleration.draw(grid, enemy_, new RobotData(this), 320, 20, 30, 20);

      grid.setColor(Color.RED);
      lateralVelocity.draw(grid, enemy_, new RobotData(this), 370, 20, 50, 20);
