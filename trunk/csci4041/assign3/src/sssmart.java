...
public class sssmart {
   ...
   public static void main(String[] args) {
      ...
      double p = 4000; // pick somewhere to start
      double d, f_p, fprime_p;
      // 4p^3 - heapSize*p + 2n
      ...
      // **********************************************************************
      System.out.println("Using a p value of: " + (int) Math.abs(p));

      // start timing and run the sort on the given list
      long stime = System.currentTimeMillis();
      sorter.sort(list, (int) Math.abs(p));
   ...
   public void sort(double[] a, int p) {
      ...
      // size : p + n/p + 4p^2
      PWayMerge[][] bucketMerge = new PWayMerge[p][p];
      for (int i = 0; i < p; i++) {
         ...
         for (int j = 0; j < p - 1; j++) {
            ...
            bucketMerge[j][i] = new PWayMerge(privious, next - 1);
            privious = next;
         }
         bucketMerge[p - 1][i] = new PWayMerge(privious, splits[i + 1] - 1);
      }
      ...
      for (int i = 0; i < p; i++) {
         ...
         while (bheapSize > 0) {
            buckets[i][bucketSize[i]++] = a[b[0].index()];
            b[0].i++;
